...
"""Stage 4.5: Random target navigation on flat plane.

Teaches the ant to turn and walk in ANY direction, not just +x.
A visible red sphere marks the current target location.
"""
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    # Visible target marker — bright red sphere floating above ground
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        """Same observation dimension as all other stages."""
        ...
        # --- Navigation (random target reuses waypoint obs slots) ---
        ...
        # --- Obstacle Scan (DUMMY — no walls in this stage) ---
...
class RewardsCfg:
    ...
    # === Penalties — relaxed to allow free turning ===
    yaw_spin = RewTerm(
        func=custom_mdp.yaw_rate_penalty, weight=-0.01,
        params={"asset_cfg": SceneEntityCfg("robot"), "rate_cap": 2.0},
    )
    roll = RewTerm(
        func=custom_mdp.roll_penalty, weight=-0.3,
...
@configclass
class AntStage4_5RandomTargetsEnvCfg(ManagerBasedRLEnvCfg):
    """Stage 4.5: random target navigation on flat plane."""
