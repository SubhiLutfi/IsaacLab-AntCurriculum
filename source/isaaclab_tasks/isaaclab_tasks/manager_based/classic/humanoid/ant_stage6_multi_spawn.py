...
"""Stage 6: Full L-maze with multiple spawn points.

Same scene/rewards as Stage 5. Adds a second spawn point (1.5, 1.3, 0)
inside the horizontal corridor to teach robustness from different starts.
No noise reset — continues from Stage 5 best checkpoint.
"""
    ...
    reset_base = EventTerm(
        ...
        params={
            ...
            "spawn_points": [
                ...
                (1.5, 1.3, 0.0),    # mid horizontal corridor (past chicane)
                (-0.3, -1.5, 0.0),  # chicane entry near bottom wall (hard: must navigate around chicane)
                (1.2, -1.5, 0.0),   # past chicane near bottom wall, before WP0 (hard: must navigate up)
            ],
            "xy_jitter": 0.0,
            "yaw_jitter": 0.0,
            "check_radius": 0.9,
...
@configclass
class AntStage6MultiSpawnEnvCfg(ManagerBasedRLEnvCfg):
    """Stage 6: Full L-maze with 2 spawn points for robustness."""
