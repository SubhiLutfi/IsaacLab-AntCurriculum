...
"""Stage 3: Corridor with two chicane obstacles (zigzag).

Adds chicane_2 at x=4.0 on the opposite side — ant must zigzag through both.
3.5m spacing between chicanes matches the ant's dodge capability.
Identical observations and rewards to Stage 2 v3.
"""
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    chicane_2 = AssetBaseCfg(
        prim_path="{ENV_REGEX_NS}/chicane_2",
        spawn=sim_utils.CuboidCfg(
            size=(0.25, 2.2, 1.2),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                kinematic_enabled=True, disable_gravity=True
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=(4.0, 0.7, 0.6)),
    )
...
@configclass
class RewardsCfg:
    """Locomotion + obstacle avoidance rewards. Identical to Stage 2 v3."""
...
@configclass
class AntStage3TwoObstaclesEnvCfg(ManagerBasedRLEnvCfg):
    """Stage 3: corridor with two chicanes, ant must zigzag."""
