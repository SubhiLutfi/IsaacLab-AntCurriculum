...
Full observation space is defined here with dummy channels for future stages.
This ensures the network architecture is fixed across ALL training stages.
"""
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # --- Obstacle Scan (DUMMY Stage 0-1, LIVE from Stage 2) ---
        obstacle_scan = ObsTerm(
            func=custom_mdp.obstacle_scan_dummy,
            params={"num_rays": 5, "max_range": 5.0},
        )
...
@configclass
class RewardsCfg:
    """Locomotion rewards only. All obstacle/nav weights = 0."""

    progress = RewTerm(
        func=mdp.progress_reward, weight=0.5,
        params={"target_pos": (1000.0, 0.0, 0.0)},
    )
    forward_speed = RewTerm(
        func=custom_mdp.grounded_forward_speed_reward, weight=0.8,
        params={
    ...
    lateral_motion = RewTerm(
        func=custom_mdp.lateral_motion_penalty, weight=-0.35,
        params={"asset_cfg": SceneEntityCfg("robot"), "speed_cap": 0.5},
    )
    yaw_spin = RewTerm(
        func=custom_mdp.yaw_rate_penalty, weight=-0.4,
...
@configclass
class AntStage0ForwardEnvCfg(ManagerBasedRLEnvCfg):
    """Stage 0: forward locomotion with full observation architecture."""
